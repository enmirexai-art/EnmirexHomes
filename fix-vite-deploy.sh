#!/bin/bash
# Script to fix the vite issue on the VM

echo "=== Fixing vite issue on VM ==="
...
# Create a fixed Dockerfile on the VM
...
sudo docker build -f Dockerfile.fix -t enmirexai/enmirex-homes:fixed .
